...
func highlightedSwiftKeywords(of text: NSMutableAttributedString) -> NSMutableAttributedString{
    ...
    let keywords = parsed.statements.classifications.filter { $0.kind == .keyword }.map { $0.range.toNSRange() }
...
func highlightSwiftKeywords(_ text: String, textStorage: NSTextStorage?) {
    ...
    var attributedString = NSMutableAttributedString(string: text, attributes: regularTextAttributes)
...
#Preview {
    EditorTextView(text: .constant("let hello = \"world\"\nfunc check() { return \"on those who return home\" }"))
}
